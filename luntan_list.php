...
require 'session3.php';
	if (!$row=_get_one('luntan_case',$_GET['pid'])){
		ShowMsg('错误：该媒体频道不存在！','-1');
	}
	if (_get_one_tj('ltcart',"pid={$_GET['pid']} AND uid={$_SESSION['userid']} AND zt=0")){
...
//删除购物车媒体
if ($_GET['act']=='ltcart_del' && isset($_GET['cid'])){
	if (!$row=_get_one_tj('ltcart',"id={$_GET['cid']} AND uid={$_SESSION['userid']} AND zt=0")){
		ShowMsg('错误：购物车中没有该商品！','-1');
	}
	_delete('ltcart',$_GET['cid']);
	_location(NULL,$_SERVER['HTTP_REFERER']);
}

//清空购物车
if ($_GET['act']=='ltcart_all_del'){
	_delete_tj('ltcart',"uid={$_SESSION['userid']} AND zt=0");
	_location(NULL,$_SERVER['HTTP_REFERER']);
}
...
if ($_SESSION['userid']) {
	...
	while (!!$row=_mysql_list($_result)) {
		$r_html.=getDbName('luntan_case','title',$row['pid']).$row['price'].'元<a href="?act=ltcart_del&cid='.$row['id'].'">[删]</a> ';
		$z_price+=$row['price'];
	}
	$r_html.='总计'.$z_price.'元 ';
	$r_html.='<a href="?act=ltcart_all_del">【全部清空】</a> ';
	$r_html.='<a href="luntan_add.php" class="addsub">下一步，提交稿件</a>';
}else{
...
<body>
<div class="main">
  <div class="weizhibox">当前位置：软文发布管理 &gt;&gt; 选择发布论坛</div>
  <?php require 'user_top_tp.php'?>
<?php require 'user_top_gg.php'?>
<?php require 'user_top.php'?>
    <div class="add_buzhuo">
    	<a href="luntan_list.php" class="online">第一步：选择需要发布的网站论坛</a>
    	<a href="luntan_add.php">第二步：添加并提交软文稿件内容</a>
    ...
    <div class="meitiBox">
    	<ul>
        	...
        	<li class="nostyle">---》</li>
<?php
$_result=_query("SELECT * FROM luntan ORDER BY px_id ASC");
while (!!$row=_mysql_list($_result)) {
?>
        	<li><a href="?mid=<?php echo $row['id']?>"><?php echo $row['title']?></a></li>
<?php }?>
        	<li><a href="luntan_list.php">更多...</a></li>
        </ul>
    	...
    	<ul>
        	...
        	<li class="nostyle">---》</li>
<?php
$_result=_query("SELECT distinct title FROM luntan_case where hide = 0 ORDER BY title ASC");
while (!!$row=_mysql_list($_result)) {
?>
        	<li><a href="?title=<?php echo $row['title']?>"><?php echo $row['title']?></a></li>
<?php }?>
        	<li><a href="luntan_list.php">更多...</a></li>
        </ul>
        <div class="clear mb10"></div>
...
<?php
$sql="SELECT * FROM luntan_case".$sql_seach;
_page($sql,20);
...
?>
  <tr>
    ...
    <td><?php if($row['case_url']=='') :?><?php echo $row['title']?><?php else :?><a href="<?php echo $row['link']?>" target="_blank"><?php echo $row['title']?></a><?php endif ;?>   <?php if($row['case_url']!='') :?><?php endif ;?></td>
      <td><a target="_blank" href="<?php echo $row['case_url']?>"><?php echo $row['isbao']?></a></td>
      <td><a href="?mid=<?php echo $row['mid']?>"><?php echo  getluntan($row['mid'])?></a></td>
    <?php  if ($_SESSION['userid']) { ?>
  <?php if($vip['kd']=='1') {?>   
 ...
 <div style=" height:60px; clear:both; "><a  style="color:#3E5F9B;" href="/luntanlist.xls" target="_blank">下载论坛资源报价表</a></div>
