...
function cartBox(){
	...
	while (!!$row=_mysql_list($_result)) {
		$r_html.=getDbName('weixin_case','title',$row['pid']).'<em>'.$row['price'].'</em>元 ';
		$z_price+=$row['price'];
	}
...
if ($_POST['pn_post']=='立即提交稿件'){
	...
	_update_tj('wxcart',$data,"uid={$_SESSION['userid']} AND zt=0");
	...
	ShowMsg('成功：您的稿件已提交成功，金额变化-'.$z_price.'元！','weixin_admin.php');
}

if (!_get_one_tj('wxcart',"uid={$_SESSION['userid']} AND zt=0")) ShowMsg('提示：您还没有选择需要发布稿件的媒体！','-1');
...
<?php require 'user_top.php'?>
   <div class="add_buzhuo">
    	<a href="weixin_list.php">第一步：选择需要发布的网站微信</a>
    	<a href="weixin_add.php" class="online">第二步：添加并提交软文稿件内容</a>
...
<?php endif ;?> 
    <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#C9D3E9" class="xuqiu">
      <tr>
        <td width="120" height="40"><p class="mc">所选微信</p></td>
        <td><p class="mc"><?php echo $cartBoxHtml?> <a href="weixin_list.php">返回重新选择</a></p></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td>
        <p class="mc"><textarea name="content" class="editor_content" id="content" rows="10" ></textarea>
        </p>
        <p class="mc pt10">友情提示<br />1、活动标题请直接编辑在内容中(仅支持一张图片且图片不能大于2M，建议尺寸：700像素 * 300像素)<br /> 2、此处理内容编辑框可填写稿件转发链接提交</p></td>
      </tr>
        <tr>
        <td height="40"><p class="mc">阅读原文链接</p></td>
        <td><p class="mc"><textarea name="jiegao" cols="110" rows="1" id="jiegao"></textarea></p></td>
      </tr>
      <tr>
