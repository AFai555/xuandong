...
<div class="banner">
  <div class="focusBox">
    <ul class="pic" style="position: relative; width: 1366px; height: 432px; ">
          ...
          <li style="background:url(./other/banner1.jpg) no-repeat center 0px;"></li>
    </ul>
...
<div class="block2 mb10">
	<div class="box">
    	<h1 class="title">资讯公告</h1>
        <div class="boxcon mb10">
     
        	<ul>
<?php
$sql="SELECT * FROM news where hide = 1  ORDER BY px_id desc";
_page($sql,20);
$sql=$sql." LIMIT $_pagenum,$_pagesize";
$_result=_query($sql);
while (!!$row=_mysql_list($_result)) {
?>
            	<li><span></span><a href="shownews.php?sid=<?php echo $row['id']?>" target="_blank"><?php echo $row['title']?></a></li>
<?php }?>
            </ul>
    
        </div>
        <div align="center" style=" clear:both; text-align:center; padding:15px 0px; font-family:Arial, Helvetica, sans-serif; border:1px solid #EEEEEE; background:#F9FCEF; border-top:none; "><?php echo _pageshow(2)?></div>
    </div>
