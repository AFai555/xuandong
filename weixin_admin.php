...
if ($_GET['m_title']!='') {
	$sql_seach.=" AND pid in (SELECT id FROM weixin_case WHERE title like '%{$_GET['m_title']}%')";
}
...
<?php require 'user_top.php'?>
    <div class="add_buzhuo">
    	<a href="weixin_list.php">第一步：选择需要发布的微信</a>
    	<a href="weixin_add.php">第二步：添加并提交软文稿件内容</a>
    	<a href="weixin_admin.php" class="online">第三步：查看软文发布进度结果</a>
    </div>
...
<?php
$sql="SELECT * FROM wxcart".$sql_seach;
_page($sql,20);
...
?>
  <tr>
   ...
   <td><?php if ($row['zt']==3){ echo '<a href="'.$row['get_url'].'" target="_blank">已发布，查看</a>' ;}else{ echo cart_zt($row['zt']);}?></td>
    <td><?php echo $row['addtime']?></td>
    <td>
	<?php if ($row['zt']==3) echo '<a href="seeswx.php?id='.$row['id'].'" target="_blank">预览</a> <a href="'.$row['baoimg'].'" target="_blank">收录截图</a> ';?>
    
	<?php if ($row['zt']==2) echo '<a href="seeswx.php?id='.$row['id'].'" target="_blank">预览</a> ';?>
    
	<?php if ($row['zt']==4) echo '<a href="hl_see.php?id='.$row['id'].'">拒搞原因</a>  <a href="seeswx.php?id='.$row['id'].'" target="_blank">预览</a>  ';?>
	
	<?php if ($row['zt']==1) echo '<a href="weixin_edit.php?id='.$row['id'].'">修改</a>  <a href="seeswx.php?id='.$row['id'].'" target="_blank">预览</a>  ';?>
    </td>
    </tr>
 <?php }?>  
  <tr>
