...
require 'session3.php';
	if (!$row=_get_one('weibo_case',$_GET['pid'])){
		ShowMsg('错误：该媒体频道不存在！','-1');
	}
	if (_get_one_tj('wbcart',"pid={$_GET['pid']} AND uid={$_SESSION['userid']} AND zt=0")){
...
//删除购物车媒体
if ($_GET['act']=='wbcart_del' && isset($_GET['cid'])){
	if (!$row=_get_one_tj('wbcart',"id={$_GET['cid']} AND uid={$_SESSION['userid']} AND zt=0")){
		ShowMsg('错误：购物车中没有该商品！','-1');
	}
	_delete('wbcart',$_GET['cid']);
	_location(NULL,$_SERVER['HTTP_REFERER']);
}

//清空购物车
if ($_GET['act']=='wbcart_all_del'){
	_delete_tj('wbcart',"uid={$_SESSION['userid']} AND zt=0");
	_location(NULL,$_SERVER['HTTP_REFERER']);
}
...
if ($_SESSION['userid']) {
	...
	while (!!$row=_mysql_list($_result)) {
		$r_html.=getDbName('weibo_case','title',$row['pid']).$row['price'].'元<a href="?act=wbcart_del&cid='.$row['id'].'">[删]</a> ';
		$z_price+=$row['price'];
	}
	$r_html.='总计'.$z_price.'元 ';
	$r_html.='<a href="?act=wbcart_all_del">【全部清空】</a> ';
	$r_html.='<a href="weibo_add.php" class="addsub">下一步，提交稿件</a>';
}else{
...
<?php require 'user_top.php'?>
    <div class="add_buzhuo">
    	<a href="weibo_list.php" class="online">第一步：选择需要发布的网站媒体</a>
    	<a href="weibo_add.php">第二步：添加并提交软文稿件内容</a>
...
<?php }?>
        	<li><a href="weibo_list.php">更多...</a></li>
        </ul>
    	...
    	<ul>
        	<li>粉丝量</li>
        	<li class="nostyle">---》</li>
        	<li><a href="?isbao=1">0-5万</a></li>
        	<li><a href="?isbao=2">5-10万</a></li>
        	<li><a href="?isbao=3">10-50万</a></li>
        	<li><a href="?isbao=4">50-100万</a></li>
        	<li><a href="?isbao=5">100-300万</a></li>
        	<li><a href="?isbao=6">300-500万</a></li>
        	<li><a href="?isbao=7">500万以上</a></li>
        	<li><a href="weibo_list.php">更多...</a></li>
        </ul>
        <div class="clear"></div>
  </div>
...
?>
  <tr>
    ...
    <td><a href="?mid=<?php echo $row['mid']?>"><?php echo  getweibo($row['mid'])?></a></td>
  
    <td><?php echo $row['isbao']?>万</td>
   <?php  if ($_SESSION['userid']) { ?>
   <?php if($vip['kd']=='1') {?>   
...
<div style=" height:60px; clear:both; "><a  style="color:#3E5F9B;" href="/weibolist.xls" target="_blank">下载微博资源报价表</a></div>
