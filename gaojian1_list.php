...
if ($_SESSION['userid']) {
	...
	while (!!$row=_mysql_list($_result)) {
		$r_html.=getDbName('meiti_case','title',$row['pid']).$row['price'].'元<a href="?act=cart_del&cid='.$row['id'].'">[删]</a> ';
		$z_price+=$row['price'];
	}
	$r_html.='总计'.$z_price.'元 ';
...
<?php }?>
  ...
  <tr>
    ...
    <th width="12%">分类</th>
    <th width="8%">会员价</th>
    <th width="18%">链接[仅供参考]</th>
    <th width="12%">收录[仅供参考]</th>
    <th width="20%">备注</th>
  </tr>
<?php
...
<div style=" height:60px; clear:both; "><a  style="color:#3E5F9B;" href="http://cs168512.113.72bz.net/ziyuan.xls">下载此分类报价表</a></div>
