...
if ($_GET['act']=='del' && isset($_GET['id'])) {
	_query("DELETE FROM wxcart WHERE id={$_GET['id']} LIMIT 1");
	ShowMsg('成功：稿件成功删除','weixin.php');
	exit();
}
...
function cart_member($uid){
	...
	return '<a href="gaojianwx.php?uid='.$row['id'].'">'.$row['my_username'].'</a>';
}  
$sql_search="";
...
if ($_GET['mid']!=''){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE mid={$_GET['mid']})";
}
if ($_GET['uid']!=''){
	$sql_search.=" AND uid={$_GET['uid']}";
}
if ($_GET['m_title']!=''){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE title like '%{$_GET['m_title']}%')";
}
if ($_GET['cb_price']!=''){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE cb_price='{$_GET['cb_price']}')";
}
if ($_GET['qq']!=''){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE qq='{$_GET['qq']}')";
}
if ($_GET['tel']!=''){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE tel='{$_GET['tel']}')";
}
if ($_GET['admin_id']!=''){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE mid in (SELECT id FROM weixin WHERE admin_id={$_GET['admin_id']}))";
}
if ($_SESSION['lv']==2){
	$sql_search.=" AND pid in (SELECT id FROM weixin_case WHERE mid in (SELECT id FROM weixin WHERE admin_id={$_SESSION['admin']}))";
}
?>
...
$z_price=0;
$z_cb_price=0;
$sql="SELECT * FROM wxcart WHERE zt>0".$sql_search." ORDER BY id DESC";
_page($sql,$listnum);
...
if ($row['zt']<4){
$z_price+=$row['price'];
$z_cb_price+=getDbName('weixin_case','cb_price',$row['pid']);
}
?>
    ...
    <tr>
      ...
      <td width="5%" align="center"><strong style="color:#FF0000;"><?php echo getDbName('weixin_case','cb_price',$row['pid'])?>元</strong></td>
      ...
      <a href="gaojianwx_see.php?id=<?php echo $row['id']?>">[查看]</a>
