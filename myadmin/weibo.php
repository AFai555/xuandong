...
if ($_GET['act']=='del' && isset($_GET['id'])) {
	_query("DELETE FROM weibo WHERE id={$_GET['id']} LIMIT 1");
	ShowMsg('成功：友情链接删除','weibo.php');
	exit();
}
...
if ($_GET['act']=='checkbox' & $_GET['lx']==3) {
	if (isset($_POST['ID'])) {
		...
		_query("DELETE FROM weibo WHERE id in ({$ids})");
		ShowMsg('成功：批量删除成功','weibo.php');
		exit();
	}else{
...
<body>
<div class="menubox"><strong>当前位置：</strong>微博 &gt; 微博管理</div>
<div class="adminbox">
<div id="menu">
<dl>
	<dt class="dropdown"><span><a href="weibo_add.php">添加微博</a></span>
    </dt>
</dl>
</div>
...
<?php
$sql="SELECT * FROM weibo ORDER BY px_id ASC";
_page($sql,12);
...
?>
    <tr>
      ...
      <td width="15%" align="center"><a href="weibo_edit.php?id=<?php echo $row['id']?>" class="icon_edit">编辑</a> <a href="?act=del&id=<?php echo $row['id']?>" class="icon_del" onclick="delcfm()">删除</a></td>
    </tr>
<?php }?> 
  </table>
</form>
