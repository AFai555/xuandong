...
if ($_GET['act']=='del' && isset($_GET['id'])) {
	_query("DELETE FROM luntan_case WHERE id={$_GET['id']} LIMIT 1");
	ShowMsg('成功：论坛案例删除成功',$_SERVER['HTTP_REFERER']);
}
...
if ($_GET['act']=='checkbox' & isset($_GET['lx'])) {
	if (isset($_POST['ID'])) {
		...
		if ($_GET['lx']==3) {
			_query("DELETE FROM luntan_case WHERE id in ({$ids})");
		}
...
<body>
<div class="menubox"><strong>当前位置：</strong>论坛 &gt; 论坛案例管理</div>
<div class="adminbox">
<form action="" method="get" name="form2" id="form2">
<div class="search_box">论坛频道：
...
?>
    <tr>
      ...
      <td align="left"><div class="newstitle"><a href="<?php echo $row['link']?>" target="_blank"><?php echo $row['title']?></a> <a href="<?php echo $row['case_url']?>" target="_blank">【<?php echo $row['isbao']?>】</a></div></td>
      <td width="6%" align="center"><?php echo getluntan($row['mid'])?></td>
      ...
      <td width="25%" align="center"><?php echo $row['beizhu']?></td>
      <td width="10%" align="center"><a href="luntancase_edit.php?id=<?php echo $row['id']?>" class="icon_edit">编辑</a> <a href="?act=del&id=<?php echo $row['id']?>" class="icon_del" onclick="delcfm()">删除</a></td>
    </tr>
<?php }?> 
  </table>
</form>
