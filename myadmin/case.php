...
if ($_GET['act']=='del' && isset($_GET['id'])) {
	_query("DELETE FROM meiti_case WHERE id={$_GET['id']} LIMIT 1");
	ShowMsg('成功：案例删除成功',$_SERVER['HTTP_REFERER']);
}
...
if ($_GET['act']=='checkbox' & isset($_GET['lx'])) {
	if (isset($_POST['ID'])) {
		...
		if ($_GET['lx']==3) {
			_query("DELETE FROM meiti_case WHERE id in ({$ids})");
		}
...
<body>
<div class="menubox"><strong>当前位置：</strong>媒体 &gt; 案例管理</div>
<div class="adminbox">
