...
function cart_member($uid){
	...
	return '<a href="gaojian.php?uid='.$row['id'].'">'.$row['my_username'].'</a>';
}          
...
if ($_GET['act']=='upd' && isset($_GET['id'])) {
	
	_query("update cart set fkbj = 1 WHERE id={$_GET['id']} LIMIT 1");
	ShowMsg('成功：付款标记成功','gaojianbb.php');
}
...
if ($_GET['mid']!=''){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE mid={$_GET['mid']})";
}
if ($_GET['uid']!=''){
	$sql_search.=" AND uid={$_GET['uid']}";
}
if ($_GET['m_title']!=''){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE title like '%{$_GET['m_title']}%')";
}
if ($_GET['cb_price']!=''){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE cb_price='{$_GET['cb_price']}')";
}
if ($_GET['qq']!=''){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE qq='{$_GET['qq']}')";
}
if ($_GET['tel']!=''){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE tel='{$_GET['tel']}')";
}
if ($_GET['admin_id']!=''){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE mid in (SELECT id FROM meiti WHERE admin_id={$_GET['admin_id']}))";
}
if ($_SESSION['lv']==2){
	$sql_search.=" AND pid in (SELECT id FROM meiti_case WHERE mid in (SELECT id FROM meiti WHERE admin_id={$_SESSION['admin']}))";
}
?>
...
<input name="listnum" type="text" id="listnum" value="20" style="width:20px;" />
  ...
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#CCE0F6" class="newstitletable">
    <tr>
      ...
      <td width="8%" align="center">是否对账</td>
...
$z_price=0;
$z_cb_price=0;
$sql="SELECT * FROM cart WHERE zt>0 and zt <> 4 ".$sql_search." ORDER BY id DESC";
_page($sql,$listnum);
...
?>
    ...
    <tr>
      ...
      <strong style="color:#990066;"> <?php  
	  
	   if (cart_bao($row['pid']) ==1||cart_bao($row['pid']) ==2)
	   {
		   if($row['baoimg']=='index.php')
		   {
			    echo '<a href="bao.php?id='.$row['id'].'"></a>';
			   }else
			   {
				    echo '<a href="bao.php?id='.$row['id'].'"></a>';
				   }
      ...
      <td width="14%" align="center"><?php echo $row['addtime']?></td>
       <td width="8%" align="center"><strong style="color:#FF0000;">
	   <?php
	    if ($row['fkbj']==1)
		{
	    echo "已付款";
      ...
      <a href="?act=upd&id=<?php echo $row['id']?>">[付款标记]</a>
