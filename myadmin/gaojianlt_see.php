...
if ($_GET['act']=='ju' && isset($_GET['id'])){
	if (!$row=_get_one_tj('ltcart',"id={$_GET['id']} AND (zt=1 or zt=2)")) ShowMsg('错误：该稿件不存在','-1');
	$data['zt']=4;
	_update('ltcart',$data,$_GET['id']);
...
if ($_POST['pn_post']=='提交网址' && isset($_POST['gid'])){
	if (!$row=_get_one('ltcart',$_POST['gid'])) ShowMsg('错误：该稿件不存在','-1');
	$g_zt=$row['zt'];
	$data['get_url']=$_POST['get_url'];
	$data['zt']=3;
	if ($data['get_url']=='') ShowMsg('错误：提交的网址不能为空','-1');
	$gid=$_POST['gid'];
	_update('ltcart',$data,$gid);
...
if ($_POST['pn_postx']=='确定修改频道' && isset($_POST['gidx'])){
	if (!$row=_get_one('ltcart',$_POST['gidx'])) ShowMsg('错误：该稿件不存在','-1');	
	...
	$gidx=$_POST['gidx'];
	_update('ltcart',$data,$gidx);
	ShowMsg('成功：已成修改频道！',$_SERVER['HTTP_REFERER']);
}

if ($_GET['act']=='shou' && isset($_GET['id'])){
	if (!$row=_get_one('ltcart',$_GET['id'])) ShowMsg('错误：该稿件不存在','-1');
	$data['zt']=2;
	_update('ltcart',$data,$_GET['id']);
...
<div class="adminbox">
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#EEEEEE">
    <tr>
      ...
      <td><p class="see_con"><span style="float:right;"><a href="/seelt.php?<?php echo encrypt($row['id'], 'E')?>" target="_blank">【查看链接】</a></span><strong><?php echo $row['title']?></strong></p></td>
    </tr>
    <tr>
...
<?php endif ;?>
  ...
  <tr>
    <td style="overflow-x:hidden;overflow-y:hidden; word-wrap:break-word;">
      ...
      <tr>
      <td height="35"><p class="see_con">类型</p></td>
