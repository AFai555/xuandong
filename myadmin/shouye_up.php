...
if ($_GET['act']=='xiugai' ) {
	...
	$data2['imgurl']=$_POST['img'];
	_update('logo_up',$data2,'2');


	
	ShowMsg('成功：上传成功了！',$PreviousUrl);
}
$row2=_get_one('logo_up',2);
...
<input name="PreviousUrl" type="hidden" id="PreviousUrl" value="<?php echo $_SERVER['HTTP_REFERER']?>" />
    <dl>
      ...
      <dt><em>首页大图：  </em>
        ...
        <input name="img" type="text" id="img" size="60" class="up_pic_input" value="<?php echo $row2['imgurl']?>"/>
  ...
  <img style="width:900px;" src="<?php echo $row2['imgurl']?>"/>
