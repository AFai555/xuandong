...
if ($_GET['act']=='del' && isset($_GET['id'])) {
	_query("DELETE FROM weixin_case WHERE id={$_GET['id']} LIMIT 1");
	ShowMsg('成功：微信案例删除成功',$_SERVER['HTTP_REFERER']);
}
...
if ($_GET['act']=='checkbox' & isset($_GET['lx'])) {
	if (isset($_POST['ID'])) {
		...
		if ($_GET['lx']==3) {
			_query("DELETE FROM weixin_case WHERE id in ({$ids})");
		}
...
<body>
<div class="menubox"><strong>当前位置：</strong>微信 &gt; 微信案例管理</div>
<div class="adminbox">
<form action="" method="get" name="form2" id="form2">
<div class="search_box">微信频道：
...
?>
    <tr>
      ...
      <td align="left"><div class="newstitle"><?php echo $row['title']?>【<?php echo $row['isbao']?>】</div></td>
      <td width="6%" align="center"><?php echo getweixin($row['mid'])?></td>
      ...
      <td width="10%" align="center"><a href="weixincase_edit.php?id=<?php echo $row['id']?>" class="icon_edit">编辑</a> <a href="?act=del&id=<?php echo $row['id']?>" class="icon_del" onclick="delcfm()">删除</a></td>
    </tr>
<?php }?> 
  </table>
</form>
